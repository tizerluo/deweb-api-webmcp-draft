...
- A **discovery file** `/.tape/api.json` so a container’s endpoint (`#<#ID>@<processor number>`) can list methods. (Both drafts flag two pre-existing constraints to maintainers: `/.tape/` is gateway-reserved — SPEC §7.8 — and readable vanity names don’t exist yet.)
- Optional **payment** as TAP-10 attachments / a future escrow — **not required** for WebMCP.
- A **tool list** `/.tape/mcp.json` so TapeKit can register WebMCP tools (`document.modelContext`, Chrome origin trial) on a real origin (an official gateway such as tapekit.org, HashPort, or self-hosted). Sandbox iframes cannot be WebMCP providers.
